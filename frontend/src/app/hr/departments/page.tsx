    ...
    <DashboardLayout title="Departments" subtitle="Manage organizational departments">
      ...
      <Modal isOpen={showModal} onClose={closeModal} title={editing ? 'Edit Department' : 'Add New Department'} size="sm">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {/* Department ID — read-only, populated by system */}
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-gray-700">Department ID</label>
            <input
              readOnly
              value={editing ? editing.id : 'Auto-generated by system'}
              className="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-100 bg-gray-50 text-gray-400 cursor-not-allowed select-none"
            />
            {!editing && <p className="text-xs text-gray-400">A unique ID will be assigned automatically when the department is created.</p>}
          </div>
          <Input
