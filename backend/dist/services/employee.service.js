...
class EmployeeService {
    ...
    async createEmployee(data) {
        ...
        await database_1.default.salaryStructure.create({
            data: {
        ...
        const mandatoryCourses = await database_1.default.course.findMany({ where: { is_mandatory: true }, select: { id: true } });
        if (mandatoryCourses.length > 0) {
            await database_1.default.courseEnrollment.createMany({
    ...
    async updateSalaryStructure(employeeId, data) {
        return database_1.default.salaryStructure.upsert({
    ...
    async getDashboardStats(employeeId) {
        ...
        const [attendanceCount, pendingLeave, leaveBalances, todayAttendance] = await Promise.all([
            ...
            database_1.default.leaveRequest.count({ where: { employee_id: employeeId, status: 'PENDING' } }),
            database_1.default.leaveBalance.findMany({
                where: { employee_id: employeeId },
                include: { leave_type: true },
            }),
            database_1.default.attendance.findFirst({
